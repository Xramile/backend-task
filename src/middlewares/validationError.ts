...
export const catchValidationError = (
  ...
  if (!result.isEmpty()) {
    res.status(400).json(responseFactory.fail(result.array()));
  } else {
    next();
  }
};
...
export const catchValidationErrorForUpdates = (
  ...
  if (!result.isEmpty()) {
    ...
    if (errors.some((err) => err.msg === 'NO_UPDATE')) {
      return res.status(400).json(responseFactory.fail('NO_UPDATES'));
    }

    return res.status(400).json(responseFactory.fail(errors));
  }
  next();
};
